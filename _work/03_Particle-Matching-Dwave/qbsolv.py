...
def QBSolve_quantum_solution(Q, token, print_energy=False):
    ...
    # response = EmbeddingComposite(DWaveSampler(token=token, solver='DW_2000Q_6'))\
    #     .sample_qubo(Qdict, num_reads=1000, chain_strength=np.amax(np.absolute(Q))*1.1)

    sample = DWaveSampler(token=token, solver='Advantage_system4.1')

    response = EmbeddingComposite(sample).sample_qubo(Qdict, num_reads=1000, answer_mode='raw', chain_strength=np.amax(np.absolute(Q))*1.1)
    ...
    num_oc = list(response.data_vectors['num_occurrences'])
    min_energy_index = np.argmin(energies)

    print("min energy index: ", min_energy_index)


    qb_solutions = list(response.samples())

    qb_solution_list = list([qb_solutions[i].values() for i in range(len(qb_solutions))])

    # converting the qb_solution_list to a list of lists
    qb_solution_list = [list(i) for i in qb_solution_list]

    return qb_solution_list, energies, num_oc
